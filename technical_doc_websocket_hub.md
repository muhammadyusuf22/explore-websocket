...
graph TD
    ...
    Flutter <--> Gateway
    Web <--> Gateway
    VO -- Internal HTTP/Redis --> Gateway
    BO -- Internal HTTP/Redis --> Gateway
    PM -- Internal HTTP/Redis --> Gateway
...
### 7.2 Flowchart: Pengiriman Notifikasi (Service to Client)

```mermaid
sequenceDiagram
    participant S as Internal Service (Voucher/Biller)
    participant H as WebSocket Hub (Controller)
    participant HS as WebSocket Hub (Service)
    participant HG as WebSocket Hub (Gateway)
    participant R as Redis Backplane
    participant C as Mobile/Web Client

    Note over S, C: Alur Pengiriman Notifikasi Real-time
    S->>H: POST /internal/events/emit (Payload)
    H->>HS: Process Event Data
    HS->>HG: server.to('user:123').emit(event)
    HG-->>R: PubSub (Distribution across instances)
    R-->>HG: Receive on targeted instance
    HG->>C: Socket.io Emit to Client Room
```
